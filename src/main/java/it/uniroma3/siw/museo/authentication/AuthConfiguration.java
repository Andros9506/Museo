...
public class AuthConfiguration extends WebSecurityConfigurerAdapter
{
	...
	protected void configure(HttpSecurity http) throws Exception
	{
		http
			...
			.antMatchers(HttpMethod.GET, "/","/index", "/home", "/info", "/collezione/{id}", "/artista", "/artista/{id}", "/css/**", "/images/**", "/collezioni", "/artisti", "/opere", "/opera/{id}", "/login", "/periodi", "/opereAnno/{dataRealizzazione}", "/artistiRicerca").permitAll()
			...
			.and().formLogin()
			.loginPage("/login")
			// se il login ha successo si viene reindirizzati al path /home
			.defaultSuccessUrl("/default")
			// logout
			.and().logout()
			// logout attivato con una richiesta Get a "/logout"
			.logoutUrl("/logout")
			// se il logout ha successo si viene reindirizzati al path /home
